...
function getNeighbour(squares, i, j, direction) {
    // controllare che esista? per custom mode TODO
    if (direction === 'n') {
        if (squares[i - 1][j].s === 0) {
            return false;
        }
        squares[i - 1][j].s = 0;
...
function getNeighbourAdd(squares, i, j, direction) {
    // controllare che esista? per custom mode TODO
    if (direction === 'n') {
        if (squares[i - 1][j].s === 1) {
            return false;
        }
        squares[i - 1][j].s = 1;
        squares[i - 1][j].maxConnect ++;
